...
for i in `seq 1 ${LOOP_COUNT}`; do
  ...
  mysql --host 0.0.0.0 --port 3306 --user=${DB_USER} --password=${DB_PASS} ${DB_NAME} < ${SQL_DIR}/load-test__initialize.sql
  log "INFO" "check database, purchase_history table and product table"
  mysql --host 0.0.0.0 --port 3306 --user root --password=mypassword web_scraper <<EOF
    select count(*) from product; select count(*) from purchase_history;
EOF

  log "INFO" "execute batch=purchase_history"
  java -DtacticFile=${TACTIC_PATH} -jar ${JAR_PATH} --batch=purchase_history --site=amazon

  log "INFO" "NOT execute batch=product"
