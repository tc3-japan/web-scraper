...
public class RakutenLoginHandler extends LoginHandlerBase {
  ...
  @Autowired
  public RakutenLoginHandler(ECSiteAccountRepository ecSiteAccountRepository, UserRepository userRepository,
  ...
  public LoginResponse login(int userId, LoginRequest request) throws ApiException {
    ...
    RakutenAuthenticationCrawler crawler = new RakutenAuthenticationCrawler(this.getECSite(),
        applicationContext.getBean(WebpageService.class));
    TrafficWebClient webClient = new TrafficWebClient(userId, false);

    try {
      RakutenAuthenticationCrawlerResult result = crawler.authenticate(webClient, request.getEmail(),
